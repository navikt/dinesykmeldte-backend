    ...
    FunSpec({
        ...
        test("klassifiserer tokenhenting uten å logge vilkårlig exception") {
            ...
            coEvery { accessTokenClient.getAccessToken(any()) } throws
                IllegalStateException(tokenExceptionCanary)
        ...
        test("mapper PDL-koder deterministisk til lukket error_code-katalog") {
            ...
            assertEquals(
                ...
                listOf(
                    ResponseError(message = "General GraphQL error"),
...
private fun responseError(code: String): ResponseError =
    ResponseError(extensions = ErrorExtension(code = code))
...
private fun pdlResponseWithDataAndError(
    ...
    dataIdent: String,
    localContext: String,
    hasUsableAktorId: Boolean,
): String {
    val identGroup = if (hasUsableAktorId) "AKTORID" else "FOLKEREGISTERIDENT"
    return """
        {
          "errors": [
            {
